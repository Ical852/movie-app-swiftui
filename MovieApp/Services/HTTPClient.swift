...
class HTTPClient {
    ...
    func getMovieDetailsBy(imdbId: String, completion: @escaping (Result<MovieDetail?, NetworkError>) -> Void) {
        ...
        guard let url = URL.forMoviesByImdbId(imdbId)
        else {
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            guard let movieDetailsResponse = try? JSONDecoder().decode(MovieDetail.self, from: data)
            else {
                return completion(.failure(.decodingError))
            }
            
            completion(.success(movieDetailsResponse.self))
